...
public class TestMain {
    ...
    public void hasDefaults() {
        assertEquals(true, new Main(new String[0]).options.get("p") != null);
        assertEquals(true, new Main(new String[0]).options.get("d") != null);
    }

    @Test
    public void canSetPort() {
        assertEquals("5000", new Main(new String[] { "-p", "5000" }).options.get("p"));
    }

    @Test
    public void canSetDirectory() {
        assertEquals("/volume", new Main(new String[] { "-d", "/volume" }).options.get("d"));
    }
    ...
    public void listenForConnection() throws IOException {
        ...
        Main main = new Main(new ServerSocketFactoryMock(mock), new String[0]);
    ...
    public void handlesBadSocketServer() throws Exception {
        Main main = new Main(new BadServerSocketFactoryMock(), new String[0]);
        main.start();
    ...
    public void handlesBadSocket() throws Exception {
        Main main = new Main(new ServerSocketThrowsErrorFactoryMock(), new String[0]);
        main.start();
    }
    ...
    public void handlesBadThreads() throws Exception {
        String text = "GET /foobar HTTP/1.1\r\nHost: google.com\r\nType: text/plain\r\n\r\n";
        SocketMock mock = new SocketMock(text);
        Main main = new Main(new BadThreadFactory(), new String[0]);
